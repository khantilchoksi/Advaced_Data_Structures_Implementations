...
int main(int argc, char** argv)
{
    ...
    if( ( inputFP = fopen( "input.bin", "rb" )) != NULL){
        ...
        initial_load_runs(runs, runsFP, input_buffer, blockSize, currentBufferRunPointers, runBufferReadCount, isRunExhausted);
        ...
        //Merge Runs
        printf("\n 291 LINE RUNS ARE: %d", runs);
        
        printf("\n TEST OUTPUT ----------\n");
        
        for(i = 0 ; i<10; i++){
            printf("\n currentBufferRunPointers: %d, runBufferReadCount: %d, isRunExhausted: %d",currentBufferRunPointers[i],runBufferReadCount[i],isRunExhausted[i]);
        }
