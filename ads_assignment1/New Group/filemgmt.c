...
void addRecordAtOffset(FILE *fp, long offset, char* record){
    ...
    //printf("\n IN FUNC: Writing Record: %s of length : %ld",record, recordLength);
    fwrite(&recordLength, sizeof(int), 1, fp);
    fwrite(record, recordLength, 1, fp);
}
...
//void printStudentFile(avail_S *array, int totalHoles){
//    printf("Availability:\n");
//    int totalHoleSizeAvailable = 0;
//    for(int i = 0 ; i < totalHoles; i++){
//        //printf(" Hole Size: %d , Offset: %ld\n", array[i].siz, array[i].off);
...
int main(int argc, char** argv)
{
    ...
    char *studentFileName = argv[2];
    //take student db in argument
    if ( ( studentDbFP = fopen( studentFileName, "r+b" ) ) == NULL ){
        studentDbFP = fopen( studentFileName, "w+b" );
    }
        ...
        if(strcmp(command,addCommand) == 0){
            ...
            if(foundIndex != -1){
                ...
                if(foundAvailbleHoleIndex == -1) {
                    ...
                    //printf("\n Writing Record: %s of length : %d",record, strlen(record));
                    ...
                    if(availableHoles[foundAvailbleHoleIndex].siz > requiredHoleSize){
                        ...
                        long newHoleOffset = offset + requiredHoleSize;     //check this???????
